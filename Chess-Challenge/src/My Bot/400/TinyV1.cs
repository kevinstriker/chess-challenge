...
public class TinyV1 : IChessBot
{
  ...
  public Move Think(Board board, Timer timer)
  {
    ...
    int SquareBonus(int piece, int position, bool isWhite)
    {
      int square = position ^ (isWhite ? 56 : 0);
      return (int)((packed[square / 16 + (piece * 4)] >> (square % 16 * 4)) & 15) - 7;
    }
      ...
      foreach (bool isWhite in new[] { !board.IsWhiteToMove, board.IsWhiteToMove })
      {
        ...
        for (var pieceIndex = 0; ++pieceIndex <= 6;)
        {
          ...
          while (bitboard != 0)
          {
            ...
            // Pieces in the middle of the board be better
            score += SquareBonus(pieceIndex - 1, sq, isWhite) * 2;
          }
        }
      }
      return score;
